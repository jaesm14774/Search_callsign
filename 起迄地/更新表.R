...
#更新既有的callsign表
b=read.csv('C:/data/test_key/key.csv',
...
Update_all=function(callsign){
  ...
  for(i in 1:length(B)){
    ...
    #刪除"更多" 無意義列
    p=which(grepl(pattern = '更多|註冊會員',x=tab[,1]))
    if(length(p)>0){
      tab=tab[-p,]
    }else tab=tab
    ...
    if(length(deter) == 0){
      ...
      code='Big5'
      temp=
        read.csv(file = paste0(path=save_path,b_unrevise[i],
                               '.csv'),stringsAsFactors = F,
                 h=T,fileEncoding = code)
      #flightaware 有可能把預先的般排進去，但時間表可能會有違差別，
      #因此先刪除前兩筆數據，以免重複導入(如果資料集>2筆數據)
      if(dim(temp)[1]>5){
        temp=temp[-c(1:5),]
      }else if(dim(temp)[1]>3){
        temp=temp[-c(1:3),]
      }else{
        temp=temp
      }
      Sys.setlocale(category='LC_ALL')
      #網站防護機制(?不確定為什麼不定期有亂碼產生，重跑一次就沒有
      temp=
        tryCatch(rbind(temp,tab),
                 error=function(e){
                   Sys.sleep(10)
                   Sys.setlocale(locale='chinese')
                   url=
                   ...
                   return(rbind(temp,tab))
                 })
      #上面一大串是防亂碼機制，不確定有沒有用
      tab=temp[!duplicated(temp[,1:4],fromLast = T),]
      tab[[4]]=tab[[4]] %>% as.POSIXct() 
      tab=tab %>% arrange(.,desc(起飛))
      tab[[4]]=as.character(tab[[4]])
      tab=distinct(tab,起飛,飛行時間,.keep_all=T)
      rownames(tab)=1:dim(tab)[1]
      write.csv(paste0(path=save_path,b_unrevise[i],'.csv'),
                x = tab,row.names = F,fileEncoding = code)
    }
    Sys.setlocale(category='LC_ALL')
    print(i)
